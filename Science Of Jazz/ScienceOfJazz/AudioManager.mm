...
// warns if the status is not zero
void checkStatus(OSStatus s) {
	if (s) {
//		NSLog(@"WARNING (AudioManager): status = %d",s);
...
static OSStatus rioRenderCallback(void *inRefCon, 
	...
	if (inBytesTotal != bufferList->mBuffers[0].mDataByteSize) {
		NSLog(@"AudioManager.rioRenderCallback: BufferList reallocing to %d bytes",(unsigned int)inBytesTotal);
...
/*
-(id)initWithLogFrequencyBins:(int)numBins {
	if (self = [self init]) {
		convertToLogFreq = YES;
		numLogFreqBins = numBins;
	}
	return self;
}
 */
